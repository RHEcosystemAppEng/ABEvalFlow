# Task: Red Hat Lightspeed Agent Evaluation

<!-- SSO NOTE: The deployed agent uses SKIP_JWT_VALIDATION=true (mock SSO).
     Tasks are designed for unauthenticated access. Real credentials will be
     provided in a future iteration to enable deeper evaluation. -->

You are evaluating the Red Hat Lightspeed Agent, an AI assistant specialized
in Red Hat Insights and advisory services. Conduct the following conversation
with the agent, sending each message in sequence and recording all responses.
