    ...
    for attempt in range(1, max_retries + 1):
        ...
        raw = llm_client.chat_completion(
            messages=[
                ...
                {
                    "role": "user",
                    "content": ASE_EVALS_PROMPT.format(
                        skill_name=skill_name,
        ...
        tagged_evals = {"_generated_by": "ai", **evals_data}
        evals_path.write_text(json.dumps(tagged_evals, indent=2) + "\n")
        logger.info(
...
def _generate_via_api(
    ...
    (submission_dir / "instruction.md").write_text(
        ("<!-- Generated by AI -->\n\n" + instruction_text) if instruction_text.strip() else instruction_text
    )
    ...
    (tests_dir / "test_outputs.py").write_text(("# Generated by AI\n" + test_text) if test_text.strip() else test_text)
    ...
    if metadata.skip_llm_judge:
        ...
        (tests_dir / "llm_judge.py").write_text(
            ("# Generated by AI\n" + judge_text) if judge_text.strip() else judge_text
        )
...
def _generate_via_agent(
    ...
    if agent_type != "api":
        logger.warning(
            "Agent mode '%s' executes with elevated permissions — ensure the submission source is trusted",
            agent_type,
        )
...
def _generate_via_oracle(
    ...
    if not oracle_dir.is_dir():
        raise ValueError(f"Oracle mode requires an oracle/ directory in {submission_dir}")
...
def _correction_pass(
    ...
    _issues_text = "\n".join(f"- {issue}" for issue in issues)  # Reserved for prompt use
    ...
    if instruction_path.is_file():
        ...
        if corrected.strip():
            instruction_path.write_text("<!-- Generated by AI -->\n\n" + corrected)
    ...
    if test_path.is_file():
        ...
        if corrected.strip():
            test_path.write_text("# Generated by AI\n" + corrected)
...
def generate(
    ...
    for attempt in range(1, max_retries + 1):
        ...
        if struct_errors:
            ...
            logger.warning(
                "Attempt %d structural validation failed: %s",
                attempt,
                struct_errors,
            )
            if attempt == max_retries:
                raise ValueError(
                    f"Generation failed structural validation after {max_retries} attempts: {struct_errors}"
                )
            continue
        ...
        if collect_errors:
            ...
            logger.warning(
                "Attempt %d pytest collect failed: %s",
                attempt,
                collect_errors,
            )
            if attempt == max_retries:
                raise ValueError(f"Generation failed pytest collect after {max_retries} attempts: {collect_errors}")
            continue
        ...
        if scenario_brief is not None:
            ...
            if coherence_errors:
                ...
                logger.warning(
                    "Attempt %d coherence check failed: %s",
                    attempt,
                    coherence_errors,
                )
                if attempt == max_retries:
                    raise ValueError(
                        f"Generation failed coherence check after {max_retries} attempts: {coherence_errors}"
                    )
                continue
        ...
        if not review["passed"]:
            ...
            if post_errors:
                ...
                if attempt == max_retries:
                    raise ValueError(
                        f"Generation failed post-correction validation after {max_retries} attempts: {post_errors}"
                    )
                continue
